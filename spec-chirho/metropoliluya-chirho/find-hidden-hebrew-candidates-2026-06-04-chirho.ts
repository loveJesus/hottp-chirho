...
function spanReasonsChirho(spanChirho: SpanChirho, lineChirho: SpanLineChirho, spansChirho: SpanChirho[], indexChirho: number): string[] {
  ...
  if (DIGIT_CLUSTER_RE_CHIRHO.test(textChirho) && !BENIGN_NUMERIC_CONTEXT_RE_CHIRHO.test(textChirho)) {
...
function renderReportChirho(candidatesChirho: CandidateChirho[], reportPathChirho: string): string {
...
function mainChirho(): void {
  ...
  const candidatesChirho = scanSpanLinePathsChirho()
  ...
  writeFileSync(reportPathChirho, renderReportChirho(candidatesChirho, reportPathChirho));
  console.log(
    JSON.stringify(
      {
