...
const SHORT_LATIN_QUESTION_GARBAGE_IN_LINE_RE_CHIRHO = /(?:^|[\s([{])(?:[A-Za-z]{1,2}\?)(?=$|[\s,.;:)\]}])/u;
...
function spanReasonsChirho(spanChirho: SpanChirho, lineChirho: SpanLineChirho, spansChirho: SpanChirho[], indexChirho: number): string[] {
  ...
  if (
    isAdjacentToHebrewChirho(spansChirho, indexChirho) &&
    SHORT_LATIN_QUESTION_GARBAGE_IN_LINE_RE_CHIRHO.test(textChirho) &&
    !BENIGN_NUMERIC_CONTEXT_RE_CHIRHO.test(numericContextTextChirho)
  ) {
    reasonsChirho.push("short-latin-question-garble-adjacent-to-hebrew-chirho");
  }
...
function scoreCandidateChirho(lineChirho: SpanLineChirho, lineTextValueChirho: string, suspiciousSpansChirho: SuspiciousSpanChirho[]): {
  ...
  const hasShortLatinQuestionGarbleChirho = suspiciousSpansChirho.some((spanChirho) =>
    spanChirho.reasonsChirho.includes("short-latin-question-garble-adjacent-to-hebrew-chirho")
  );
  ...
  if (hasShortLatinQuestionGarbleChirho) {
    scoreChirho += 3;
    reasonsChirho.push("line-has-short-latin-question-garble-chirho");
  }
