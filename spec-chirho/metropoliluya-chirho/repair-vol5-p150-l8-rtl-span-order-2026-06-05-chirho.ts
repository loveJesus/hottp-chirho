...
/**
 * Dry-run-first repair for vol 5 p150 line 8.
 *
 * The line is a mixed French/Hebrew apparatus line. The current span geometry
 * boxes are correct, but S3/S7 text was assigned in logical Hebrew order rather
 * than to the physical target boxes. This script makes each span text match its
 * box and opts this verified slash-separated RTL run into logical rendering.
 */
...
const MODULE_CHIRHO = "repair-vol5-p150-l8-rtl-span-order-2026-06-05-chirho";
const TARGET_PATH_CHIRHO = spanLinePathChirho(5, 150, 8);
...
const S3_NOTES_CHIRHO =
  "Corrected vol 5 p150 line 8 S3 after packet crop inspection: the S3 target box is on the leftmost Hebrew word `קְבָרִים`, not the previously stored `רוּחִי חֻבָּלָה`. Stored as vision-chirho because this is a machine-witnessed geometry/text reassignment; exact letters, niqqud, and the slash-separated RTL run remain Hebrew/WLC expert-confirmation tier.";
const S7_NOTES_CHIRHO =
  "Corrected vol 5 p150 line 8 S7 after packet crop inspection: the S7 target box is on the rightmost Hebrew phrase `רוּחִי חֻבָּלָה`, not the previously stored `קְבָרִים`. The line carries lineTextOrderChirho=rtl-runs-logical-chirho so exported Markdown keeps the logical Hebrew run order while span text matches the boxed geometry. Stored as vision-chirho pending Hebrew/WLC expert confirmation.";
...
function validateTargetLineChirho(lineChirho: SpanLineChirho): void {
  if (lineChirho.volumeChirho !== 5 || lineChirho.pageChirho !== 150 || lineChirho.lineIndexChirho !== 8) {
    throw new Error("target line identity mismatch");
  }
  if (lineChirho.lineWidthPxChirho !== 2046) throw new Error(`line width ${lineChirho.lineWidthPxChirho} !== expected 2046`);
...
function stateChirho(lineChirho: SpanLineChirho): "pre-repair-chirho" | "already-applied-chirho" | "unknown-chirho" {
  ...
  if (
    s3Chirho.xMinPxChirho === 917 &&
    s3Chirho.widthPxChirho === 258 &&
    s3Chirho.scriptChirho === "hebrew-chirho" &&
    s3TextChirho === "רוּחִי חֻבָּלָה" &&
    s3Chirho.provenanceChirho === undefined &&
    s5Chirho.xMinPxChirho === 1246 &&
    s5Chirho.widthPxChirho === 354 &&
    s5Chirho.scriptChirho === "hebrew-chirho" &&
    s5TextChirho === "יָמַי נִזְעָכוּ" &&
    s7Chirho.xMinPxChirho === 1671 &&
    s7Chirho.widthPxChirho === 375 &&
    s7Chirho.scriptChirho === "hebrew-chirho" &&
    s7TextChirho === "קְבָרִים" &&
...
function plannedLineChirho(lineChirho: SpanLineChirho, appliedAtChirho: string): SpanLineChirho {
  ...
  s3Chirho.utf8TextChirho = normalizeTextForStorageChirho("קְבָרִים");
  s3Chirho.provenanceChirho = "vision-chirho";
  s3Chirho.visionTranscribedAtChirho = appliedAtChirho;
  s3Chirho.visionCorrectionStatusChirho = APPLIED_STATUS_CHIRHO;
  s3Chirho.visionNotesChirho = S3_NOTES_CHIRHO;
  s7Chirho.utf8TextChirho = normalizeTextForStorageChirho("רוּחִי חֻבָּלָה");
  s7Chirho.provenanceChirho = "vision-chirho";
  ...
  return [
    {
      ...
      garbleTextChirho: "previously assigned `רוּחִי חֻבָּלָה` to the S3 box",
    ...
    },
    {
      ...
      garbleTextChirho: "previously assigned `קְבָרִים` to the S7 box",
      scriptChirho: "hebrew-chirho",
