...
- [x] Have sync-out exclude duplicate derived SQLite files outside the canonical
      `spec-chirho/` DB set.
...
- [x] Have the write-lease runbook and template require stopped review servers
      rather than paused writers before sync-out or pull-back.
...
- [x] Have SSH host preflight rerun the VPS readiness guard from the synced
      host tree before smoke completion can pass.
