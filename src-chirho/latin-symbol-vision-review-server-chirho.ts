...
<body>
  <main class="shell-chirho">
    ...
    <div class="toolbar-chirho">
      ...
      <input id="exact-text-filter-chirho" type="text" placeholder="optional exact current text" />
  ...
  <script>
    ...
    function activeItemsChirho() {
      ...
      return itemsChirho.filter((itemChirho) =>
        ...
        (exactTextFilterChirho === "" || itemChirho.textChirho === exactTextFilterChirho)
      );
    }
    ...
    function applyRequestedItemIdChirho() {
      ...
      if (requestedIndexChirho < 0 && requestedItemChirho) {
        ...
        if (symbolRiskFilterChirho !== "all-chirho" && requestedItemChirho.symbolRiskChirho !== symbolRiskFilterChirho) {
          symbolRiskFilterChirho = "all-chirho";
          changedFiltersChirho = true;
        }
        ...
        if (exactTextFilterChirho !== "" && requestedItemChirho.textChirho !== exactTextFilterChirho) {
    ...
    const commonSymbolNameByValueChirho = new Map([
      ["≠", "not equal sign"],
      ["+", "plus sign"],
      ["/", "solidus"],
      [":", "colon"],
    ...
    function rangeCodepointNamePartChirho(codepointChirho) {
      if (codepointChirho >= 0x1D400 && codepointChirho <= 0x1D7FF) return " mathematical alphanumeric symbol";
      if (codepointChirho >= 0x00C0 && codepointChirho <= 0x024F) return " Latin letter with diacritic/extension";
      if (codepointChirho >= 0xFB1D && codepointChirho <= 0xFB4F) return " Hebrew presentation form";
      if (codepointChirho >= 0x1F00 && codepointChirho <= 0x1FFF) return " Greek polytonic";
      if (codepointChirho >= 0x0370 && codepointChirho <= 0x03FF) return " Greek";
      if (codepointChirho >= 0x0590 && codepointChirho <= 0x05FF) return " Hebrew";
      return "";
    }
    function codepointNamePartChirho(charChirho) {
      const directNameChirho =
        ...
        commonSymbolNameByValueChirho.get(charChirho) ??
      ...
      if (decomposedChirho.length > 1) {
        const partsChirho = decomposedChirho
          .map((partChirho) =>
            ...
            commonSymbolNameByValueChirho.get(partChirho)
          )
          .filter((partChirho) => typeof partChirho === "string" && partChirho.length > 0);
        if (partsChirho.length > 0) return " " + partsChirho.join(" + ");
      }
      return rangeCodepointNamePartChirho(charChirho.codePointAt(0));
    }
    ...
    function renderChirho() {
      ...
      leftChirho.appendChild(elChirho("div", { classChirho: "text-box-chirho mono-chirho codepoints-chirho", textChirho: codepointTextChirho(itemChirho.textChirho) }));
