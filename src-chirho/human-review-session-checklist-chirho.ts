...
export function humanReviewSessionChecklistLinesChirho(
  ...
  return [
    ...
    `- 1. Attribution cleanup (${inputChirho.attributionRowCountChirho} row(s)): use unchanged-live-text reattribution only for rows genuinely attributable to the named human reviewer (${inputChirho.attributionUnchangedRowCountChirho} unchanged; ${inputChirho.attributionChangedRowCountChirho} changed). Unchanged lane: ${inputChirho.attributionUnchangedLaneUrlChirho}; changed/re-review lane: ${inputChirho.attributionRereviewLaneUrlChirho}`,
    `- 2. Raw Hebrew certification: start with the primary unvalidated lane and inspect the crop plus full line before any clean save. Clean saves need the clean-certification checkbox; dots inside letters are vowels/niqqud, while cantillation/meteg are accent/meteg issues. Start: ${inputChirho.rawHebrewPrimaryLaneUrlChirho}`,
    `- 3. Hebrew/Greek expert confirmations: confirm only exact letters, marks, punctuation, spacing, and crop against the print; use Report issue or Skip when uncertain. Hebrew: ${inputChirho.expertHebrewLaneUrlChirho}; Greek: ${inputChirho.expertGreekLaneUrlChirho}`,
    ...
    `- 5. Latin/symbol proofing: after script-critical work, review French, Latin, witness sigla, references, and nontrivial symbols against the print; witness sigla and references are not blanket-safe. Start: ${inputChirho.latinSymbolLaneUrlChirho}`,
  ];
}
