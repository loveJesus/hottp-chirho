...
function latinAcceptedSafeSymbolArgsChirho(outPathChirho: string, extraArgsChirho: string[] = []): string[] {
  ...
  return [
    ...
    "--rationale-chirho=guard confirms every selected trivial punctuation item was checked clean",
...
function expertConfirmedArgsChirho(
  ...
  return [
    ...
    "--reviewer-chirho=dr-policy-prep-expert-human-chirho",
    `--reviewer-role-chirho=${reviewerRoleChirho}`,
    "--rationale-chirho=guard confirms the exact printed item after expert visual review",
    `--out-chirho=${outPathChirho}`,
