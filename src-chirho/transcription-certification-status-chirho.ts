...
function markdownChirho(statusChirho: CertificationStatusChirho): string {
  ...
  return [
    ...
    "- Certification-affecting saves use the configured trusted server-side reviewer identity (`HOTTP_TRUSTED_REVIEWER_HEADER_CHIRHO`, or local `--reviewer` fallback when that header is absent); the browser reviewer field is display-only for stored attribution.",
