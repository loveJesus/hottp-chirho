...
const SMOKE_RUNBOOK_SNIPPETS_CHIRHO = [
  ...
  "public traffic must go through Cloudflare to Caddy",
...
function assertProvisioningDecisionHelperChirho(): void {
  ...
  for (const snippetChirho of [
    ...
    "review DNS records must be proxied when DNS changes are approved",
    "raw-review.bible.systems",
...
function assertSmokeEvidenceHelperChirho(): void {
  ...
  for (const snippetChirho of [
    ...
    "QUARANTINE_PROGRESS_DB_PATH_CHIRHO",
    "source-local-fixture-quarantine-db-chirho",
    "commit-back proof missing quarantined SQLite snapshot",
