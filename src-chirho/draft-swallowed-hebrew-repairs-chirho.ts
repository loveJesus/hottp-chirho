...
function draftProposalsChirho(storePathChirho: string): SegmentRepairProposalRecordChirho[] {
  ...
  for (const specChirho of REPAIR_SPECS_CHIRHO) {
...
function triageRowsChirho(
...
): { rowsChirho: TriageRowChirho[]; escalatedSuspectsChirho: WitnessRecordChirho[] } {
  const draftedKeysChirho = new Set(REPAIR_SPECS_CHIRHO.map((specChirho) => specChirho.itemKeyChirho));
  ...
  const escalatedSuspectsChirho = witnessRecordsChirho.filter(
    (recordChirho) =>
  ...
  return { rowsChirho, escalatedSuspectsChirho };
}

function renderTriageReportChirho(
  rowsChirho: TriageRowChirho[],
  escalatedSuspectsChirho: WitnessRecordChirho[],
  witnessRecordsChirho: WitnessRecordChirho[],
  ...
  const linesChirho: string[] = [
    ...
    `- Stored-Hebrew-suspect escalations (disjoint confident read, vols 3-5 CRNN-weak rule): ${escalatedSuspectsChirho.length}`,
    "",
    "## Drafted Proposals",
    "",
    ...
    "",
    ...escalatedSuspectsChirho.map(
      (recordChirho) =>
        `- ${recordChirho.spanKeyChirho}: read ${recordChirho.readSkeletonChirho} (conf ${(recordChirho.crnnConfChirho ?? 0).toFixed(2)}, P(hebrew) ${(recordChirho.v8PHebChirho ?? 0).toFixed(2)}) vs stored — disjoint; confirm on the crop.`
    ),
    "",
...
function mainChirho(): void {
  ...
  const candidatesChirho = JSON.parse(readFileSync(CANDIDATES_PATH_CHIRHO, "utf8")) as {
    findingsChirho: SweepFindingChirho[];
  };
  const witnessChirho = JSON.parse(readFileSync(WITNESS_PATH_CHIRHO, "utf8")) as {
    recordsChirho: WitnessRecordChirho[];
  };
  const draftedChirho = draftProposalsChirho(storePathChirho);
  const triageChirho = triageRowsChirho(candidatesChirho.findingsChirho, witnessChirho.recordsChirho);
  writeTextAtomicChirho(
    reportPathChirho,
    renderTriageReportChirho(
      triageChirho.rowsChirho,
      triageChirho.escalatedSuspectsChirho,
  ...
  console.log(
    `[${MODULE_CHIRHO}] drafted=${draftedChirho.length} decisions=${JSON.stringify(countsChirho)} escalatedSuspects=${triageChirho.escalatedSuspectsChirho.length} report=${relative(PROJECT_ROOT_CHIRHO, reportPathChirho)} store=${relative(PROJECT_ROOT_CHIRHO, storePathChirho)}`
  );
}
